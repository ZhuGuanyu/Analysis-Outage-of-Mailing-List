...
for filename in dirs:
	...
	or filename == "tt" or filename == "test.py" or filename==".DS_Store" or filename == "findKeyword.py":
		continue
	if str(year) not in filename and "New" in filename:
			...
			print("################# Type of outages ###################")
			print(str(year-1) + " Routing = " + str(routing) + " Percentage = " + str(float(routing)/float(count)))
			print(str(year-1) + " Power Outage = " + str(power) + " Percentage = " + str(float(power)/float(count)))
			print(str(year-1) + " Packet loss = " + str(packet) + " Percentage = " + str(float(packet)/float(count)))
			print(str(year-1) + " Natural disaster = " + str(natural) + " Percentage = " + str(float(natural)/float(count)))
			print(str(year-1) + " Fiber cut = " + str(fiber) + " Percentage = " + str(float(fiber)/float(count)))
			print(str(year-1) + " Attacks = " + str(attack) + " Percentage = " + str(float(attack)/float(count)))
			print(str(year-1) + " Censorship = " + str(censorship) + " Percentage = " + str(float(censorship)/float(count)))
			print(str(year-1) + " Maintiance = " + str(maintiance) + " Percentage = " + str(float(maintiance)/float(count)))
			...
			totalfiber+=fiber
			totalattack+=attack
			totalcensorship+=censorship
			totalmaintiance+=maintiance
	...
	with open(filename, "r") as f:
		...
		for index in range(len(lines)):
			...
			if "Google" in lines[index] or "google" in lines[index]:
				google+=1
			if "att " in lines[index]:
				if "matt" not in lines[index] and "gigawatt" not in lines[index]:
					att+=1
			...
			if "BGP" in lines[index] or "bgp" in lines[index]:
				bgp+=1
			if "dns" in lines[index] or "dns resolution":
				dns+=1 
			...
			if "TCP" in lines[index] or "tcp" in lines[index]:
				tcp+=1
			if "Routing" in lines[index] or "routing" in lines[index]:
				routing+=1
			if "Power outage" in lines[index] or "power outage" in lines[index] or "power outages" in lines[index]:
				power+=1
			if "Packet loss" in lines[index] or "packet loss" in lines[index]:
				packet+=1
			if "earthquake" in lines[index] or "storm" in lines[index] or\
			"tornado" in lines[index] or "quake" in lines[index]:
				natural+=1
			if "fiber cut" in lines[index] and "storm" not in lines[index] and "packet loss" not in lines[index]:
				fiber+=1
			if "ddos" in lines[index] or "dos" in lines[index]:
				attack+=1
			if "censor" in lines[index] or "block" in lines[index]:
				censorship+=1
			if "maintain" in lines[index] or "maintenance" in lines[index]:
				maintiance+=1
	'''
...
print("################# Type of outages ###################")
print(str(year) + " Routing = " + str(routing) + " Percentage = " + str(float(routing)/float(count)))
print(str(year) + " Power Outage = " + str(power) + " Percentage = " + str(float(power)/float(count)))
print(str(year) + " Packet loss = " + str(packet) + " Percentage = " + str(float(packet)/float(count)))
print(str(year) + " Natural disaster = " + str(natural) + " Percentage = " + str(float(natural)/float(count)))
print(str(year) + " Fiber cut = " + str(fiber) + " Percentage = " + str(float(fiber)/float(count)))
print(str(year) + " Censorship = " + str(censorship) + " Percentage = " + str(float(censorship)/float(count)))
print(str(year) + " Maintiance = " + str(maintiance) + " Percentage = " + str(float(maintiance)/float(count)))
...
print "==============	Routing ==============="
print "Routing = " + str(totalrouting)
print "Power outages = " + str(totalpower)
print "Congestion = " + str(totalpacket)
print "Natural disaster = " + str(totalnatural)
print "Fiber cut = " + str(totalfiber)
print "Attacks = " + str(totalattack)
print "Censorship = " + str(totalcensorship)
print "Maintiance = " + str(totalmaintiance)
print "DNS resolution = " + str(totaldns)
