...
"""
	1. terminal的指令
	./svmlin -A 3 -W 0.001 -U 1 -R 0.5 "RealTime/train_data.txt" "RealTime/train_label" + str(i+1)+ ".txt"
	跑程序后会生成两个文件
	train_data.txt.weights
	train_data.txt.outputs
	2. 然后terminal的指令
	./svmlin -f  train_data.txt.weights "RealTime/test_label_data.txt" "RealTime/test_label" + str(i+1) + ".txt"
	得到文件
	test_label_data.txt.outputs
	3.terminal指令
	./svmlin -f  train_data.txt.weights RealTime/test_unlabel_data.txt
	得到文件
	test_unlabel_data.txt.outputs
	4.重命名4个文件为
	train_data.txt.weights
	train_data.txt.outputs
	test_label_data.txt.outputs
	test_unlabel_data.txt.outputs
	为
	"train_data" + "str(i+1)" + ".txt.weights"
	"train_data" + "str(i+1)" + ".txt.outputs"
	"test_label_data" + "str(i+1)" + ".txt.outputs"
	"test_unlabel_data" + "str(i+1)" + ".txt.outputs"
	并移动到新的文件夹realtime_result
"""
