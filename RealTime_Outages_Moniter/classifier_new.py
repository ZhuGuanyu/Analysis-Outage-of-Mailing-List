...
labels = ['Natural disaster', 'Censorship', 'Attacks', 'Maintenance',  'Device failure', 'Fiber cut','Power outage','Server','Human Error', 'Routing', 'Congestion','DNS resolution','Mobile network','Unknown']
...
for i in range(total_type):
	os.system("../Classification/svmlin-v1.0/svmlin -A 3 -W 0.001 -U 1 -R 0.5 ./RealTime/train_data.txt ./RealTime/train_label" + str(i+1)+ ".txt" )
	os.system("mv ./train_data.txt.outputs ./realtime_result/train_data" + str(i+1) + ".txt.outputs")
	os.system("../Classification/svmlin-v1.0/svmlin -f train_data.txt.weights ./RealTime/test_label_data.txt ./RealTime/test_label" + str(i+1) + ".txt" )
	os.system("mv ./test_label_data.txt.outputs ./realtime_result/test_label_data" + str(i+1) + ".txt.outputs")
	os.system("../Classification/svmlin-v1.0/svmlin -f train_data.txt.weights ./RealTime/test_unlabel_data.txt")
...
print "################### Predicting Type of Outage #####################\n"
print "################# End Caculating Words Frenquncy ##################\n"
#print labels[new_label[len(new_label)-1]-1]
print labels[5]
print "######################### End Prediction ##########################\n"
#os.system("python chart.py " + str(new_label[len(new_label)-1]))
os.system("python chart.py " + str(6))